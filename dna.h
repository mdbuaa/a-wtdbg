...
static inline void print_seq_basebank_with_buff(BaseBank *bnk, u8i off, u8i len, FILE *out, char* _buf, int* _cur, int bsize){
	u8i i, b, e;
	if(bsize-*_cur<101) { //printf("========== fput %d size =========\n",*_cur);
		fputs(_buf, out);
		*_cur = 0;
	}
	for(b=off;b<off+len;){
...
#ifdef FAST_PRINTGRAPH
static inline void print_revseq_basebank_with_buff(BaseBank *bnk, u8i off, u8i len, FILE *out, char* _buf, int* _cur, int bsize){
	u8i i;
	if(*_cur + 65 >= bsize) { //printf("========== fput %d size =========\n",*_cur);
		fputs(_buf, out);
		*_cur = 0;
	}
	char* buf = _buf+*_cur;
	for(i=0;i<len;++i){
		buf[i] = bit_base_table[bits2revbit(bnk->bits, off + len - 1 - i)];
		++(*_cur);
		if(*_cur + 65 >= bsize) { //printf("========== fput %d size =========\n",*_cur);
			fputs(_buf, out);
			*_cur = 0;
