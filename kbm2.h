...
} kbm_baux_t;//koff:off in the hashlist
...
typedef struct {
	...
	u8i      boff;
	u8i      last_bidx;
} KBMDP;//this is for all-vs-all alignment
...
typedef struct {
	...
	BitVec   *solids;
	String   *str;
} KBMAux;//augment: contains the all-vs-all alignment, along with the intermediate result
...
static inline KBM* init_kbm(KBMPar *par){
	...
	kbm->tag2idx = init_cuhash(1023);//hashset length is set to 1023 at first
	kbm->bins  = init_kbmbinv(64);//bin: kbmbinv*, has 64 kbmbin
...
static inline void push_kbm(KBM *kbm, char *tag, int taglen, char *seq, u4i seqlen){
	...
	rd = next_ref_kbmreadv(kbm->reads);//one read one sequence
	rd->rdoff = kbm->rdseqs->size; //the head address
	rd->rdlen = seqlen;
	rd->binoff = 0;
	rd->bincnt = 0;
	rd->tag = ptr;
	seq2basebank(kbm->rdseqs, seq, seqlen);//all the sequence in one char*
	// make sure rdoff is even(oushu)
...
static inline void ready_kbm(KBM *kbm){
	...
	u4i i, j;
	if((kbm->flags & 0x2) == 0){
		if(kbm->par->rd_len_order){//default: yes
			sort_array(kbm->reads->buffer, kbm->reads->size, kbm_read_t, num_cmpgt(b.rdlen, a.rdlen));//sort from longest to shortest
		}
		encap_basebank(kbm->rdseqs, KBM_BSIZE);//update the capacity and add KBM_BSIZE to total length, this shall be the data of bases
	}
	clear_kbmbinv(kbm->bins);
	for(i=0;i<kbm->reads->size;i++){
		rd = ref_kbmreadv(kbm->reads, i);
		if(rd->tag) put_cuhash(kbm->tag2idx, (cuhash_t){rd->tag, i});//set the read into cuhash list, finally the cuhash list's elements' val is when the read first appears
		if((kbm->flags & 0x2) == 0) rd->binoff = kbm->bins->size;//default: 0
		for(j=0;j+KBM_BIN_SIZE<=rd->rdlen;j+=KBM_BIN_SIZE){
			push_kbmbinv(kbm->bins, (kbm_bin_t){i, j / KBM_BIN_SIZE, 0, 0});//bins is set in the last phase of data input, i stands for which read this bin appears, j stands for the head address of bin, and closed along with degree is set to 0
		}
		if((kbm->flags & 0x2) == 0) rd->bincnt = j / KBM_BIN_SIZE;//how many bins
	}
	clear_bitvec(kbm->binmarks);
	encap_bitvec(kbm->binmarks, kbm->bins->size);//update the capacity
...
static inline void split_FIXP_kmers_kbm(BaseBank *rdseqs, u8i offset, u4i length, u1i ksize, u1i psize, u4i kmod, kmeroffv *rs[2]){
	kmer_off_t *kp;//default: ksize 0 psize 21 kmode:4096*4 
	u8i kmer, krev, hv, npz, kmask, p, pmask;
	u4i ki, npl;//defualt: rdseq: the total seq offset: the true position, len:KBM_BIN_SIZE(256)
	int i, j;
	...
	pshift = (psize - 1) << 1;//default: 40
	pmask  = (1LLU << (psize << 1)) - 1LLU;//default: 1 in last 41 bits
	kpshf  =  psize << 1;
	if(ksize){//ksize == 0
	...
	} else if(psize){//always this branch
		kmer = krev = 0;
		b = 4;
		for(i=j=0;i<(int)length;i++){
			...
			j ++;
			if(j < psize) continue;
			if(kmer < krev){//decide the direction
				push_kmeroffv(rs[0], (kmer_off_t){kmer, i - (psize - 1), 0, 0, 0});//what is this?
			} else if(krev < kmer){
	...
	for(b=0;b<2;b++){
		for(i=0;(u4i)i<rs[b]->size;i++){
			...
			hv = kbm_kmer_smear(rs[b]->buffer[i].kmer);//smear:mohu
			ki = hv % kmod;
			if(ki >= KBM_N_HASH) rs[b]->buffer[i].closed = 1;
			rs[b]->buffer[i].kidx = ki;//a linked list. closed means ending
		}
	}
}
...
kidxs = malloc(KBM_N_HASH * sizeof(kbmmidxv*)); //4096*kbmmidxv
for(i=0;i<KBM_N_HASH;i++) kidxs[i] = init_kbmmidxv(64); //4096*64*kbmmidxv
bms = init_tmpbmerv(KBM_MAX_KCNT);
thread_beg_loop(midx);
if(midx->task == 1){
	// counting kmers
	for(i=0;i<KBM_N_HASH;i++) clear_kbmmidxv(kidxs[i]);
	for(bidx=midx->beg+tidx;bidx<midx->end;bidx+=ncpu){ //tidx: thread_id, maybe can in gpu?
		if(KBM_LOG == 0 && tidx == 0 && ((bidx - midx->beg) % 100000) == 0){ fprintf(KBM_LOGF, "\r%u", bidx - midx->beg); fflush(KBM_LOGF); } //not necessary
		bin = ref_kbmbinv(kbm->bins, bidx);//get the bidxth bin in kbm->bins
		if(bin->closed) continue; //default: closed==0
		if(((u4i)bin->off + 1) * KBM_BIN_SIZE > kbm->reads->buffer[bin->ridx].rdlen) continue;//bins cannot be longer than readlength
		off = kbm->reads->buffer[bin->ridx].rdoff + bin->off * KBM_BIN_SIZE;//the true position of this bin
		len = KBM_BIN_SIZE;//i think the split_FIXP is used for aggregate 256/psize bits randomly. 
		split_FIXP_kmers_kbm(kbm->rdseqs, off, len, kbm->par->ksize, kbm->par->psize, kbm->par->kmer_mod, kmers);//default: k 0 p 21 kmer_mod:4096*4
		for(i=0;i<2;i++){ //kmers[0]:forward kmers[1]:reverse 
			for(j=0;j<kmers[i]->size;j++){
				f = ref_kmeroffv(kmers[i], j);
				if(f->closed) continue;
				mx = next_ref_kbmmidxv(kidxs[f->kidx]); //add kidx and fetch
				mx->mer = f->kmer;
				mx->kidx = f->kidx;//shall means mix
				mx->bidx = bidx;
				mx->dir  = i;//0:forward 1:reverse
				mx->koff = f->off;
				if(kidxs[f->kidx]->size >= 64){
					...
					for(k=0;k<kidxs[kidx]->size;k++){
						...
						u = prepare_kbmhash(kbm->hashs[kidx], mx->mer, &exists);//check whether it is in hashset
						if(exists){
	...
	// stat kmer counts
	memset(midx->cnts, 0, midx->n_cnt * sizeof(u8i)); //n_cnt: MAX
	for(i=tidx;i<KBM_N_HASH;i+=ncpu){
		reset_iter_kbmhash(kbm->hashs[i]);
		while((u = ref_iter_kbmhash(kbm->hashs[i]))){
			midx->cnts[num_min(midx->n_cnt, u->tot) - 1] ++;//how many selected k-mers has occured in cnts[j] times 
		}
	}
} else if(midx->task == 4){
	// stat counts
	...
	// fill seeds, link the k-mer with bins
	for(i=0;i<KBM_N_HASH;i++) clear_kbmmidxv(kidxs[i]);
	u4v *chgs;
	...
	for(bidx=midx->beg+tidx;bidx<midx->end;bidx+=ncpu){
		...
		len = KBM_BIN_SIZE;
		split_FIXP_kmers_kbm(kbm->rdseqs, off, len, kbm->par->ksize, kbm->par->psize, kbm->par->kmer_mod, kmers);//the same as task = 1? what is used for?
		clear_u4v(chgs);
		for(i=0;i<2;i++){
			for(j=0;j<kmers[i]->size;j++){
				f = ref_kmeroffv(kmers[i], j);
		...
		for(i=0;i<chgs->size;i++){
			...
			for(k=0;k<kidxs[kidx]->size;k++){
				...
				if(u && u->flt == 0){
					x = ref_kbmkauxv(kbm->kauxs[kidx], offset_kbmhash(kbm->hashs[kidx], u));//offset amount the offset is in the kbmkauxv
					kbm->bins->buffer[mx->bidx].degree ++;//a kbmkauxv is for a kbmhash
					if(x->cnt < u->tot){
						if(x->cnt && getval_bidx(kbm, x->off + x->cnt - 1) == mx->bidx && kbm->sauxs->buffer[x->off + x->cnt - 1].dir == mx->dir){
							// repeated kmer within one bin
						} else {
							kbm->seeds->buffer[x->off + x->cnt].bidx = mx->bidx & MAX_U4;//first 32bit of bin index
							kbm->sauxs->buffer[x->off + x->cnt].bidx = mx->bidx >> 32;//last 32bit of bin index
	...
	for(kidx=0;kidx<KBM_N_HASH;kidx++){//get from the hashlist
		if(kidxs[kidx]->size){
			// lock hashs[kidx]
			pthread_mutex_lock(midx->locks + kidx);
			// hash adding
	...
	// sort seeds within a kmer, in the asceding order of bidx
	for(i=tidx;i<KBM_N_HASH;i+=ncpu){
		reset_iter_kbmhash(kbm->hashs[i]);
		while((u = ref_iter_kbmhash(kbm->hashs[i]))){
			x = ref_kbmkauxv(kbm->kauxs[i], offset_kbmhash(kbm->hashs[i], u));
...
static inline void index_kbm(KBM *kbm, u8i beg, u8i end, u4i ncpu, FILE *kmstat){
	...
	midx->end  = end;
	midx->cnts = NULL;
	midx->n_cnt = MAX;//0xFFFF
	midx->task = 0;
	midx->cal_degree = 0;
	midx->locks = hash_locks;//4096*pthread_mutex_t
	...
	thread_beg_iter(midx);
	thread_wait(midx);
	for(i=0;i<MAX;i++) kcnts[i] += midx->cnts[i];//accumulate, may be not necessary
	thread_end_iter(midx);
	#ifdef PRINTGRAPH
	kmstat = kmstat;
	#else
...
static inline void query_index_kbm(KBMAux *aux, char *qtag, u4i qidx, BaseBank *rdseqs, u8i seqoff, u4i seqlen){ //alignment, for each time, only one read is processed
	KBM *kbm;
	KBMPar *par;
	kbm_kmer_t *u;
	kbm_kaux_t *x;
...
#endif
	bmin = par->self_aln? kbm->reads->buffer[qidx].binoff + kbm->reads->buffer[qidx].bincnt : 0;// start from the next read
...
static inline int _backtrace_map_kbm(KBMAux *aux, int dir, kbm_path_t *p){//add a new hit to kbmmapv
	KBMDP *dp;
	kbm_map_t *hit;
	kbm_cmer_t *c;
	u8i cgoff, sidx;
