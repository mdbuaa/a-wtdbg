...
static inline void append_bitsvec(BitsVec *dst, BitsVec *src, u8i off, u8i len){
	...
	#ifndef DMMODIFY
	if(off>src->size)
	fprintf(stderr, " -- signal 11 warning: off %llu, src->size: %llu in %s -- %s:%d --\n", off, src->size, __FUNCTION__, __FILE__, __LINE__);
	#endif
	...
	while(si < se){
		#ifndef DMMODIFY
		if(si>src->size)
		fprintf(stderr, " -- signal 11 warning: si %llu, src->size: %llu in %s -- %s:%d --\n", si, src->size, __FUNCTION__, __FILE__, __LINE__);
		#endif
