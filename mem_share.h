...
static inline size_t encap_list(void **buffer, size_t e_size, size_t size, size_t cur_cap, size_t inc, int mem_zeros, size_t n_head){
	...
	if(size + inc < 0x3FFFFFFFLLU){
		cap = roundup_power2(size + inc);//update the cap
	} else {
		cap = (size + inc + 0x3FFFFFFFLLU) & (MAX_U8 << 30);
	}
	#ifdef LOADDEBUG
	fprintf(stderr, "buffer: %llu, n_head: %u, e_size: %u, cap: %llu\n", buffer, n_head, e_size,cap);
	#endif
	ptr = realloc((*buffer) - n_head * e_size, e_size * (cap + n_head));
	 //expand the list, realloc will free first and then alloc
	if(ptr == NULL){
...
#ifdef DMMODIFY //check mem status
#if defined(__linux__) || defined(__unix__) || defined(__CYGWIN__)
#define MEMORY_CHECK_INFO(log, num_index_ptr)	\
if(log){	\
    FILE *statusfile; \
	u8i totalmem; \
	u8i availmem; \
	char sbuffer[64]; \
	u8i sfreed, sbuffered, scached; \
	sfreed = sbuffered = scached = 0; \
	statusfile = open_file_for_read("/proc/meminfo", NULL); \
	while((fscanf(statusfile, "%s", sbuffer)) > 0) { \
		if (strstr(sbuffer, "MemTotal") == sbuffer) { \
		fscanf(statusfile, "%llu", &totalmem); \
		totalmem *= 1024; \
		} \
		else if (strstr(sbuffer, "MemFree") == sbuffer) { \
		fscanf(statusfile, "%llu", &sfreed); } \
		else if (strstr(sbuffer, "Buffers") == sbuffer) { \
		fscanf(statusfile, "%llu", &sbuffered); } \
		else if (strstr(sbuffer, "Cached") == sbuffer) { \
		fscanf(statusfile, "%llu", &scached); } \
	  } \
	fclose(statusfile); \
	availmem = (sfreed + sbuffered + scached) * 1024; \
	fprintf(log, "--\n");	\
	fprintf(log, "-- total memory %16.1f kB\n", totalmem / 1024.0);	\
	fprintf(log, "-- available    %16.1f kB\n", availmem / 1024.0);	\
	float ratio; \
    ratio = (double)(totalmem - availmem) / ((double) totalmem); \
	fprintf(log, "-- the memory cost ratio %f kB\n",ratio); \
	if(ratio > 0.1) \
	{ \
      float estimate_ratio = ratio * 10 - 1; \
	  *(num_index_ptr) = (int)estimate_ratio + 1; \
	} \
    else \
	{ \
      *(num_index_ptr) = 1; \
	}\
	fprintf(log,"--the kbm-parts is %d\n", *(num_index_ptr)); \
	fflush(log);	\
}	
#endif
#endif
