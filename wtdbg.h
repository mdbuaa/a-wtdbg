/*
 *
 * Copyright (c) 2011, Jue Ruan <ruanjue@gail.com>
 *
 *
...
/* 
rid: the head index of read
dir: direction 0: forward 1: reverse
beg: begin position in read
end: end position in read
closed: of no use
this is structure is along with the node
*/
typedef struct {
	u8i node;
	u8i rid:30, dir:1, beg:16, end:16, closed:1;
...
typedef struct {
	u4i begin_rid: 30;
	u4i          : 2;
	u4i begin_rid_bid: 30;//this refers to comparsion range
	u4i          : 2;
	u4i begin_rid_eid: 30;
	u4i          : 2;
	u4i end_rid: 30;
	u4i          : 2;
	u4i end_rid_bid: 30;
	...
	u4i closed   : 1;
	u4i mreadflg : 1;
	u2i copy_instruct; //from (0~7)*bestn
} dm_reg_t;
#endif //this is for batch uploading in all-vs-all alignment.
...
typedef struct {
	...
	vec_ref_t regs; //information for read
	ptr_ref_t edges[2];
} node_t;
...
static inline int map2rdhits_graph(Graph *g, kbm_map_t *hit){
	...
	rh = next_ref_rdhitv(g->rdhits);//a new read hit
	...
	rh->lnks[1].cnt = hit->cglen;//maybe can merge hit and rh 
	add = 0;//can there be paralleled?
	for(k=0;k<2;k++){
		rd = ref_readv(g->reads, rh->frgs[k].rid);
#ifdef KBM_MAP2RDHIT_QUICK
		{ // Just add it, i think this can be paralleled
...
if(mdbg->task == 1){
	...
	#else //try to copy the hits and cigars to g directly. only copy the first 500 of hits
	//if(areg->closed) continue;
	size_t copylength = 0;
	kbm_map_t *ahit;
	//pthread_mutex_lock(mdbg->hitmutex);
	//check the assigned read
	u4i now_rid;
	u4i total_readsize = kbm->reads->size;
	//clear_kbmmapv(aux->hits);
	//clear_bitsvec(aux->cigars);
	#ifdef LOADDEBUG
		fprintf(KBM_LOGF, "the areg in thread %lu\n", pthread_self());
        fprintf(KBM_LOGF, "begin_rid: %u, begin_rid_bid: %u, begin_rid_eid: %u\n", areg->begin_rid, areg->begin_rid_bid, areg->begin_rid_eid);
		fprintf(KBM_LOGF, "end_rid: %u, end_rid_bid: %u, end_rid_eid: %u\n", areg->end_rid, areg->end_rid_bid, areg->end_rid_eid);		
	#endif
	if(!areg->mreadflg)
	{
		for(now_rid = areg->begin_rid; now_rid <= areg->begin_rid_eid; now_rid++)
	    {
		
			//query_index_kbm2(aux, NULL, now_rid, kbm->rdseqs, kbm->reads->buffer[now_rid].rdoff, UInt(kbm->reads->buffer[now_rid].bincnt) * KBM_BIN_SIZE, areg->begin_rid_bid, areg->begin_rid_eid);
			query_index_kbm(aux, NULL, now_rid, kbm->rdseqs, kbm->reads->buffer[now_rid].rdoff, UInt(kbm->reads->buffer[now_rid].bincnt) * KBM_BIN_SIZE);
			map_kbm(aux);
		
		
		#ifdef LOADDEBUG
		fprintf(KBM_LOGF, "now the total hit number after read %lu has been done by thread %lu is %lu\n", now_rid, pthread_self(), aux->hits->size);
		#endif
		sort_array(aux->hits->buffer, aux->hits->size, kbm_map_t, num_cmpgt(b.mat, a.mat));
        copylength = (g->bestn)? num_min(g->bestn,aux->hits->size) : aux->hits->size;
		pthread_mutex_lock(mdbg->hitmutex);
	    for(i=0;i<copylength;i++)
	    {
		    ahit = ref_kbmmapv(aux->hits,i);
		    if(ahit->mat == 0) continue;
			#ifdef LOADDEBUG
			fprintf(KBM_LOGF, "%s: aux_cigars_size: %lu, ahit_cgoff: %lu, ahit_cglen: %lu in thread %lu", __FUNCTION__, aux->cigars->size, ahit->cgoff, ahit->cglen);
			#endif
			//pthread_mutex_lock(mdbg->hitmutex);
		    append_bitsvec(g->cigars,aux->cigars,ahit->cgoff,ahit->cglen);
		    ahit->cgoff = g->cigars->size - ahit->cglen;
		    //pthread_mutex_lock(mdbg->hitmutex);
		    rd_hit_t *ardhit = next_ref_rdhitv(g->rdhits);
		    read_t *rdq;
		    ardhit->frgs[0] = (rd_frg_t){ahit->qidx, ahit->qdir, ahit->qb, ahit->qe, 0};
		    ardhit->frgs[1] = (rd_frg_t){ahit->tidx, ahit->tdir, ahit->tb, ahit->te, 0};
		    ardhit->lnks[0].cnt = ahit->mat;
		    ardhit->lnks[1].cnt = ahit->cglen;
		    for(k=0;k<2;k++)
		    {
			    rdq = ref_readv(g->reads, ardhit->frgs[k].rid);
			    ardhit->lnks[k].idx = rdq->hits.idx;
			    ardhit->lnks[k].flg = rdq->hits.flg;
			    rdq->hits.idx = offset_rdhitv(g->rdhits, ardhit);
			    rdq->hits.flg = k;
			    rdq->hits.cnt ++;
		    }
		    //pthread_mutex_unlock(mdbg->hitmutex);
	    }
		pthread_mutex_unlock(mdbg->hitmutex);
	}
	for(now_rid = areg->end_rid_bid; now_rid <= areg->end_rid_eid; now_rid++)
	{
		if(now_rid == total_readsize - 1) break;
		//query_index_kbm2(aux, NULL, now_rid, kbm->rdseqs, kbm->reads->buffer[now_rid].rdoff, UInt(kbm->reads->buffer[now_rid].bincnt) * KBM_BIN_SIZE, areg->end_rid_bid, areg->end_rid_eid);
		query_index_kbm(aux, NULL, now_rid, kbm->rdseqs, kbm->reads->buffer[now_rid].rdoff, UInt(kbm->reads->buffer[now_rid].bincnt) * KBM_BIN_SIZE);
		map_kbm(aux);
		#ifdef LOADDEBUG
		fprintf(KBM_LOGF, "now the total hit number after read %lu has been done by thread %lu is %lu\n", now_rid, pthread_self(), aux->hits->size);
	...
	{
		for(now_rid = areg->begin_rid; now_rid <= areg->end_rid; now_rid++)
	    {
	...
	#ifdef LOADDEBUG
	fprintf(KBM_LOGF, "now the total hit number in thread %lu is %lu\n",pthread_self(), aux->hits->size);
	#endif 
	/*sort_array(aux->hits->buffer, aux->hits->size, kbm_map_t, num_cmpgt(b.mat, a.mat));
    copylength = (g->bestn)? num_min(g->bestn,aux->hits->size) : aux->hits->size;
	for(i=0;i<copylength;i++)
...
} else if(mclp->task == 2){//choose the bestn
	hitlnkv *lnks, *chks;
	read_t *rd;
	hit_lnk_t *lnk;
	u4i i;
...
static inline u8i proc_alignments_core(Graph *g, int ncpu, int raw, rdregv *regs, u4v *maps[3], char *prefix, char *dump_kbm){
	...
	#ifdef DMMODIFY
	pthread_mutex_t *hitmutex;
	hitmutex = calloc(1, sizeof(pthread_mutex_t));
	#endif
	int reset_kbm, n_cpu;
	thread_prepare(mdbg);
	if(KBM_LOG) n_cpu = 1;
	else n_cpu = ncpu;
	ic = (g->kbm->bins->size + g->num_index - 1) / g->num_index;//bin: approximately bp/6
	ie = 0;
	if(g->corr_mode){
	...
	rdflags = (!g->corr_mode && g->par->skip_contained)? init_bitvec(g->kbm->reads->size) : NULL;//default skip_contain = 0
	thread_beg_init(mdbg, n_cpu);
	#ifdef DMMODIFY
	mdbg->hitmutex = hitmutex;
	#endif
	mdbg->g = g;
	#ifndef DMMODIFY
	...
	mdbg->reg.closed = 1;
	#else
	mdbg->areg = (dm_reg_t*)calloc(1, sizeof(dm_reg_t));
	(mdbg->areg)->closed = 0;
	#endif
	mdbg->aux = init_kbmaux(g->kbm);//aug: for alignment
	if(g->rpar){
	...
	memcpy(mdbg->aux->par, g->par, sizeof(KBMPar));//aux->par == g->kbm->par originally
	mdbg->regs = regs;
	mdbg->rdflags = rdflags;//default: NULL
	mdbg->beg = 0;
	mdbg->end = 0;
	mdbg->raw = raw; //default: raw = 0
	mdbg->alno = alno;
	thread_end_init(mdbg);
	in = g->corr_mode? 1 : g->num_index;
	if(g->kbm->seeds->size){ //default: size = 0
		reset_kbm = 0;
		if(in > 1){
	...
	//fix_node = 0;
	nhit = 0;
	for(ii=0;ii<in;ii++){ //split the kbm index process
		ib = ie;
		ie = ib + ic;
		...
		nbp = ((u8i)(ie - ib)) * KBM_BSIZE;//ie,ib: number of bins, bp:base pair
		if(reset_kbm){ //for the first iteration: reset_kbm=1
		...
		}
		{
			...
			#ifdef DMMODIFY //with load balance
            //u4i batchtime = 2;
			/*u4i totalsize = (qe - qb + 1) * (qe - qb) / (2 * BATCHTIME);
            u4i batchsize = totalsize / (BATCHTIME * ncpu - 1);
			u4i now_rid = qb; //qb: 0
            u4i trans = 0;
			u2i allocguide = 0; //when reaches ncpu
			u4i uploadtime = BATCHTIME * ncpu + ncpu;
			u4i r_uploadtime = BATCHTIME * ncpu;
			size_t total_readsize = ((g->kbm)->reads)->size;
			dm_reg_t *now_reg = (dm_reg_t*)calloc(1,sizeof(dm_reg_t)) ;
			dm_reg_t *next_reg = (dm_reg_t*)calloc(1, sizeof(dm_reg_t));*/
			size_t total_readsize = qe - qb;
			u4i trans = 0;
			//u4i batchsize = 4;
			//u4i total_upload_time = (total_readsize + batchsize - 1) / batchsize;
			//u4i uploadtime = (total_upload_time + ncpu - 1) / ncpu + ncpu;
			//u4i r_uploadtime = (total_upload_time + ncpu - 1) / ncpu; 
			//try regression model
            u4i batchtime = total_readsize / ncpu / 4;

			u4i uploadtime = batchtime * ncpu + ncpu;
			u4i r_uploadtime = batchtime * ncpu;
			u4i batchsize = total_readsize / (batchtime * ncpu - 1);
			dm_reg_t *now_reg = (dm_reg_t*)calloc(1, sizeof(dm_reg_t));
			dm_reg_t *next_reg = (dm_reg_t*)calloc(1, sizeof(dm_reg_t));
			
			fprintf(KBM_LOGF, "BATCHTIME: %d\n", batchtime);
			
			u4i  startflg = 0;
			u4i  endflg = total_readsize - 1; 
			u4i  stride = batchsize / 2;
			#ifdef UPLOADDEBUG
			fprintf(KBM_LOGF, "batchsize: %u, uploadtime: %u, total_readsize: %u\n", batchsize, uploadtime, total_readsize);
			#endif
            for(;trans<uploadtime;trans++)
			{ 
				if(trans<r_uploadtime) 
				{
				...
				//now_reg = mdbg->areg;
				//allocguide = (allocguide + 1) % ncpu;
				memset(next_reg, 0, sizeof(dm_reg_t));
				//decide the idx of next batch
				//if((mdbg->areg)->closed == 0)
				//{   //first:
                    /*if(now_reg->end_rid_eid == total_readsize - 1) 
					{
						next_reg->begin_rid = now_reg->end_rid + 1;
                        next_reg->begin_rid_bid = next_reg->begin_rid + 1;
						if(next_reg->begin_rid_eid + batchsize > total_readsize - 1)
						{
							next_reg->begin_rid_eid = total_readsize - 1;
							next_reg->mreadflg = 1;
							u4i readcounter = total_readsize - 1 - next_reg->begin_rid;
							next_reg->end_rid = next_reg->begin_rid + 1;
							while(1)
							{
								if((readcounter + total_readsize - 1 - next_reg->end_rid) >= batchsize)
								{
									next_reg->end_rid_bid = next_reg->end_rid + 1;
									next_reg->end_rid_eid = next_reg->end_rid_bid + batchsize - readcounter;
									break;
								}
								else
								{   if(next_reg->end_rid == total_readsize - 1)
									{
										next_reg->end_rid_bid = next_reg->end_rid;
										next_reg->end_rid_eid = next_reg->end_rid;
										break;
									}
									readcounter += total_readsize - next_reg->end_rid;
									next_reg->end_rid++;
						...
						{
							next_reg->begin_rid_eid = next_reg->begin_rid + batchsize;
							next_reg->mreadflg = 0; //not multi-reads
                            next_reg->end_rid = next_reg->begin_rid;
							next_reg->end_rid_bid = next_reg->begin_rid_bid;
							next_reg->end_rid_eid = next_reg->begin_rid_eid;
						}
                    						
					}
					else 
					{
						next_reg->begin_rid = now_reg->end_rid;
						next_reg->begin_rid_bid = now_reg->end_rid_eid + 1;
                        if(next_reg->begin_rid_bid + batchsize > total_readsize - 1)
						{
							next_reg->mreadflg = 1;
							next_reg->begin_rid_eid = total_readsize - 1;
							next_reg->end_rid = next_reg->begin_rid + 1;
                            u4i readcounter = total_readsize - 1 - next_reg->begin_rid_bid;
							while(1)
							{
								if(readcounter + total_readsize - 1 - next_reg->end_rid >= batchsize)
								{
						...
						{
							next_reg->mreadflg = 0;
							next_reg->begin_rid_eid = next_reg->begin_rid_bid + batchsize;
                            next_reg->end_rid =  next_reg->begin_rid;
							next_reg->end_rid_bid = next_reg->begin_rid_bid;
							next_reg->end_rid_eid = next_reg->begin_rid_eid;
						}
                ...
                //decide the new areg;
				//(mdbg->areg)->closed = 1;
				//if(allocguide == ncpu - 1) {} check bitvec, the allocation of rdhit is easy.
                //if(trans < r_uploadtime && (rdflags == NULL || get_bitvec(rdflags, rid) == 0)) get next trans
				//a read must be together, last n and first n

				if(endflg - startflg + 1 >= batchsize)
				{
					next_reg->begin_rid = startflg;
					next_reg->begin_rid_bid = startflg;
					next_reg->begin_rid_eid = next_reg->begin_rid_bid + stride - 1;
					next_reg->end_rid = endflg;
					next_reg->end_rid_bid = endflg - stride + 1;
					next_reg->end_rid_eid = endflg;
					next_reg->mreadflg = 0;
					startflg += stride;
				    endflg -= stride;
				}
				else
				{
					...
					next_reg->begin_rid = startflg;
					next_reg->begin_rid_bid = startflg;
					next_reg->begin_rid_eid = endflg;
					next_reg->end_rid = endflg;
					next_reg->end_rid_eid = endflg;
					next_reg->end_rid_bid = startflg;
					startflg = endflg; 
				}
				
				if(trans < r_uploadtime && startflg!=endflg )
			    {
				    //memcpy(mdbg->areg, next_reg, sizeof(dm_reg_t));
					(mdbg->areg)->begin_rid = next_reg->begin_rid;
					(mdbg->areg)->begin_rid_bid = next_reg->begin_rid_bid;
					(mdbg->areg)->begin_rid_eid = next_reg->begin_rid_eid;
					(mdbg->areg)->end_rid = next_reg->end_rid;
					(mdbg->areg)->end_rid_bid = next_reg->end_rid_bid;
					(mdbg->areg)->end_rid_eid = next_reg->end_rid_eid;
					(mdbg->areg)->mreadflg = next_reg->mreadflg;
					(mdbg->areg)->closed = 0;
					#ifdef LOADDEBUG
					fprintf(KBM_LOGF, "reg copy ends\n");
					#endif
				    thread_wake(mdbg);
			    }
			   //memcpy(now_reg, next_reg,sizeof(dm_reg_t));
			   #ifdef UPLOADDEBUG
			   fprintf(KBM_LOGF, "the next_reg and the now_reg\n");
               fprintf(KBM_LOGF, "begin_rid: %u, begin_rid_bid: %u, begin_rid_eid: %u\n", next_reg->begin_rid, next_reg->begin_rid_bid, next_reg->begin_rid_eid);
			   fprintf(KBM_LOGF, "end_rid: %u, end_rid_bid: %u, end_rid_eid: %u\n", next_reg->end_rid, next_reg->end_rid_bid, next_reg->end_rid_eid);
			   //fprintf(KBM_LOGF, "begin_rid: %u, begin_rid_bid: %u, begin_rid_eid: %u\n", now_reg->begin_rid, now_reg->begin_rid_bid, now_reg->begin_rid_eid);
			   //fprintf(KBM_LOGF, "end_rid: %u, end_rid_bid: %u, end_rid_eid: %u\n", now_reg->end_rid, now_reg->end_rid_bid, now_reg->end_rid_eid);
			   #endif
			...
			free(next_reg);
			#else
			for(rid=qb;rid<=qe+ncpu;rid++){ //rid:read_id, for each read
				if(rid < qe){
				...
				if(mdbg->reg.closed == 0){
					...
					if(alno){
						...
						}//this whole iteration is of no use
						#endif
						end_bufferedwriter(bw);
					}
					for(i=0;i<mdbg->aux->hits->size;i++){
...
static inline void build_nodes_graph(Graph *g, u8i maxbp, int ncpu, FileReader *pws, int rdclip, char *prefix, char *dump_kbm){
	...
	rdregv *regs;
	rnkrefv *nds;
	rnk_ref_t *nd;//this is for all-vs-all alignment
	...
	raw = !((g->bestn > 0) || rdclip);//default:raw = 0, even when choose --aln-best 0
	if(g->corr_mode){
		raw = 1;
	}
