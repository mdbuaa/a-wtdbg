...
#ifdef FAST_PRINTGRAPH
#define MAX_CTGS_BUFF_SIZE 5000
static inline u8i print_ctgs_graph(Graph *g, u8i uid, u8i beg, u8i end, char *prefix, char *lay_suffix, u4i ncpu, FILE *log){
	...
	for(i=beg;i<end;i++){
		...
		len = len * KBM_BIN_SIZE;
		{
			...
			rb = snprintf(buff+size, MAX_CTGS_BUFF_SIZE-size, ">ctg%llu nodes=%llu len=%u\n", uid, (u8i)path->size + 1, len);
			if(rb<0) { 
				fputs(buff, bw->out); size = 0; buff[0]='\0'; 
				rb = sprintf(buff,">ctg%llu nodes=%llu len=%u\n", uid, (u8i)path->size + 1, len); 
			}
			size += rb;
			...
			for(j=0;j<lays->size;j++){
				...
				rb = snprintf(buff+size, MAX_CTGS_BUFF_SIZE-size, "E\t%d\tN%llu\t%c\tN%llu\t%c\n", (int)t->off * KBM_BIN_SIZE, (u8i)t->node1, "+-"[t->dir1], (u8i)t->node2, "+-"[t->dir2]);
				if(rb<0) {
					//fprintf(KBM_LOGF, "begin print buff : size=%d, buff[size]=%d\n", size, buff[size]);
					fputs(buff, bw->out); size = 0; buff[0]='\0'; 
					rb = sprintf(buff,"E\t%d\tN%llu\t%c\tN%llu\t%c\n", (int)t->off * KBM_BIN_SIZE, (u8i)t->node1, "+-"[t->dir1], (u8i)t->node2, "+-"[t->dir2]); 
				}
				size += rb;
				...
				for(c=0;c<lay->rcnt;c++){
					...
					rb = snprintf(buff+size, MAX_CTGS_BUFF_SIZE-size, "%c\t%s\t%c\t%d\t%d\t", "Ss"[reg->view], g->kbm->reads->buffer[reg->rid].tag, "+-"[reg->dir], reg->beg * KBM_BIN_SIZE, (reg->end - reg->beg) * KBM_BIN_SIZE);
					if(rb<0) {
						//fprintf(KBM_LOGF, "begin print buff : size=%d, buff[size]=%d\n", size, buff[size]);
						fputs(buff, bw->out); size = 0; buff[0]='\0'; 
						rb = sprintf(buff,"%c\t%s\t%c\t%d\t%d\t", "Ss"[reg->view], g->kbm->reads->buffer[reg->rid].tag, "+-"[reg->dir], reg->beg * KBM_BIN_SIZE, (reg->end - reg->beg) * KBM_BIN_SIZE); 
					}
					size += rb;
					if(reg->dir){
						print_revseq_basebank_with_buff(g->kbm->rdseqs, g->kbm->reads->buffer[reg->rid].rdoff + reg->beg * KBM_BIN_SIZE, (reg->end - reg->beg) * KBM_BIN_SIZE, bw->out, buff, &size, MAX_CTGS_BUFF_SIZE);
					} else {
						print_seq_basebank_with_buff(g->kbm->rdseqs, g->kbm->reads->buffer[reg->rid].rdoff + reg->beg * KBM_BIN_SIZE, (reg->end - reg->beg) * KBM_BIN_SIZE, bw->out, buff, &size, MAX_CTGS_BUFF_SIZE);
					}
					//fprintf(bw->out, "\n");
					if(size+1>=MAX_CTGS_BUFF_SIZE) {
						//fprintf(KBM_LOGF, "begin print buff : size=%d, buff[size]=%d\n", size, buff[size]);
						fputs(buff, bw->out);
						size = 0;
					}
			...
			//fprintf(KBM_LOGF, "=============== begin print buff : size=%d, MAX=%d\n", size, MAX_CTGS_BUFF_SIZE);
			fputs(buff, bw->out);
			size = 0;
...
static inline u8i print_nodes_graph(Graph *g, FILE *out){
	...
	for(i=0;i<g->nodes->size;i++){
		n = ref_nodev(g->nodes, i);
		if(n->closed){ 
			//fprintf(out, "N%llu*\t%u", i, n->regs.cnt); //reg: vec_ref_t cnt is 18-bit
			ret = snprintf(buff+size, MAX_BUFF_SIZE-size, "N%llu*\t%u", i, n->regs.cnt); //reg: vec_ref_t cnt is 18-bit
			if(ret<0) { fputs(buff, out); size = 0; buff[0]='\0'; ret = sprintf(buff,"N%llu*\t%u", i, n->regs.cnt); }
		} else {
			//fprintf(out, "N%llu\t%u", i, n->regs.cnt); //regs: count for the coverage of n
			ret = snprintf(buff+size, MAX_BUFF_SIZE-size, "N%llu\t%u", i, n->regs.cnt); //regs: count for the coverage of n
			if(ret<0) { fputs(buff, out); size = 0; buff[0]='\0'; ret = sprintf(buff,"N%llu\t%u", i, n->regs.cnt); }
		} //closed is of no use
		size += ret;
		for(j=0;j<n->regs.cnt;j++){
			r = ref_regv(g->regs, n->regs.idx + j);//n->regs.idx: the head address of reads in n
			//fprintf(out, "\t%s_%c_%d_%d", g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir], r->beg, r->end - r->beg);//tag:the name rid:the id of the read
			ret = snprintf(buff+size, MAX_BUFF_SIZE-size, "\t%s_%c_%d_%d", g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir], r->beg, r->end - r->beg);//tag:the name rid:the id of the read
			if(ret<0) { 
				fputs(buff, out); size = 0; buff[0]='\0'; 
				ret = sprintf(buff,"\t%s_%c_%d_%d", g->kbm->reads->buffer[r->rid].tag, "FR"[r->dir], r->beg, r->end - r->beg); 
			}
			size += ret;
			//if(r->closed) fputc('*', out);
			if(r->closed) ret = snprintf(buff+size, MAX_BUFF_SIZE-size,'*');
			if(ret<0) { fputs(buff, out); size = 0; buff[0]='\0'; ret = sprintf(buff,'*'); }
			size += ret;
		}//forward and reverse: the different direction in DNA 0: forward 1: reverse
		//fprintf(out, "\n");
		ret = snprintf(buff+size, MAX_BUFF_SIZE-size,'\n');
		if(ret<0) { fputs(buff, out); size = 0; buff[0]='\0'; ret = sprintf(buff,'\n'); }
		size += ret;
	}
	fputs(buff,out);
...
static inline u8i print_reads_graph(Graph *g, FILE *out){
	...
	for(i=0;i<g->kbm->reads->size;i++){
		...
		idx = rd->regs.idx;
		while(idx){
			r = ref_regv(g->regs, idx);//check the attribute of index
