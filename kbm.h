...
#ifdef DMMODIFY
static inline void query_index_kbm2(KBMAux *aux, char *qtag, u4i qidx, BaseBank *rdseqs, u8i seqoff, u4i seqlen, u4i begin_rid, u4i end_rid){ //the modified version of query_index_kbm
	KBM *kbm;
	KBMPar *par;
	kbm_kmer_t *u;
	kbm_kaux_t *x;
	...
	aux->qnbit = (aux->qnbin + 63) & 0xFFFFFFC0U;
	clear_kbmdp(aux->dps[0], aux, 0);//potential memory cost: clear process do not reduce cap
	...
	bmin = par->self_aln? kbm->reads->buffer[begin_rid].binoff : 0;
	//if(bmin < aux->bmin) bmin = aux->bmin;
	bmax = kbm->reads->buffer[end_rid].binoff + kbm->reads->buffer[end_rid].bincnt;//	default: MAX_U8
	//if(begin_rid == 0 && end_rid == kbm->reads->size - 1)
	//{
	//	bmin = par->self_aln? kbm->reads->buffer[begin_rid].binoff : 0;
    //    bmax = aux->bmax;
	//}
	#ifdef LOADDEBUG
	fprintf(KBM_LOGF, "in thread: %lu, the bmin: %lu, bmax: %llu\n", pthread_self(), bmin, bmax );
	#endif
	if(par->self_aln == 2){
		par->strand_mask = 2; // 1: forward; 2: reverse; 3: both
	}
	split_FIXP_kmers_kbm(rdseqs, seqoff, aux->slen, par->ksize, par->psize, par->kmer_mod, aux->koffs); //unit: a read
#ifdef TEST_MODE
	...
	for(i=0;i<aux->refs->size;i++){
		...
		while(ref->boff < ref->bend){
			...
			if(hidx - aux->bmoff < aux->nheap){
				#ifdef LOADDEBUG
				fprintf(KBM_LOGF, "%s: hidx: %lu, aux->bmoff: %lu, i: %lu in thread %lu\n", __FUNCTION__, hidx, aux->bmoff, i, pthread_self());
				#endif
...
static inline void query_index_kbm(KBMAux *aux, char *qtag, u4i qidx, BaseBank *rdseqs, u8i seqoff, u4i seqlen){
	...
	bmin = par->self_aln? kbm->reads->buffer[qidx].binoff + kbm->reads->buffer[qidx].bincnt : 0;
	if(bmin < aux->bmin) bmin = aux->bmin;
	bmax = aux->bmax;//default: MAX_U8
	if(par->self_aln == 2){
		par->strand_mask = 2; // 1: forward; 2: reverse; 3: both
	}
	split_FIXP_kmers_kbm(rdseqs, seqoff, aux->slen, par->ksize, par->psize, par->kmer_mod, aux->koffs); //unit: a read, get the encode of the read
#ifdef TEST_MODE
	if(par->test_mode >= 6) return;
#endif
	tot = 0;
	for(i=0;i<2;i++){ //reverse and forward
		next = 0;
		for(j=0;j<aux->koffs[i]->size;j++){
			f = ref_kmeroffv(aux->koffs[i], j);
			if(f->closed) continue;
			if(kbm->flags & (1LLU << 2)) f->kidx = 0;
			u = get_kbmhash(kbm->hashs[f->kidx], f->kmer);//find kmer in hashlist
			if(u == NULL || u->flt || u->tot < par->kmin){
				continue; //if not find: continue
			}
			x = ref_kbmkauxv(kbm->kauxs[f->kidx], offset_kbmhash(kbm->hashs[f->kidx], u));
			ref = next_ref_kbmrefv(aux->refs);//add a ref, a kmer a ref
			ref->mer = u;
			ref->aux = x;
			ref->kidx = f->kidx;
			ref->off = f->off;
			ref->dir = i; //forward or reverse
			if(par->self_aln && aux->solids){ //default: solid == NULL
				sidx = (kbm->reads->buffer[qidx].rdoff + ref->off) >> 1;
				ref->fine = get_bitvec(aux->solids, sidx);
			} else {
				ref->fine = 0;
			...
			ref->poffs[1] = aux->slen - (ref->off + (aux->par->ksize + aux->par->psize));
			ref->boff = x->off;
			ref->bend = x->off + x->cnt; // x.off & x.cnt: the offset of kmers
			ref->bidx = getval_bidx(kbm, x->off);
			ref->closed = 0;
			{
				// Refine boundray
				...
				while(ref->boff < ref->bend){ //adjust the boff
					if(ref->bidx < bmin){ //bin in other read
						ref->boff ++;
						ref->bidx = getval_bidx(kbm, ref->boff);
						continue;
					//} else if(ref->b->bidx >= bmax){
				...
				while(ref->bend > ref->boff){
					if(getval_bidx(kbm, ref->bend - 1) > bmax){ //bin in other read
						ref->bend --;
					} else {
						break;
					}
	...
	if(par->self_aln && aux->solids){ //solid: default NULL
		// Obsolete
		sort_array(aux->refs->buffer, aux->refs->size, kbm_ref_t, num_cmpgt(a.off, b.off));
		tot = 0;
		next = 0;
	...
	} else if(aux->par->ksampling < KBM_BIN_SIZE && aux->refs->size){ //ksampling: default: 256
		sort_array(aux->refs->buffer, aux->refs->size, kbm_ref_t, num_cmpgtx(a.qbidx, b.qbidx, b.bend - b.boff, a.bend - a.boff));
		tot = 0;
		for(i=j=0;i<aux->refs->size;i++){
			if(aux->refs->buffer[i].qbidx != aux->refs->buffer[j].qbidx){
		...
		} //this procedure get the match bins
		//sort_array(aux->refs->buffer, aux->refs->size, kbm_ref_t, num_cmpgt(a.off, b.off));
	}
	sort_array(aux->refs->buffer, aux->refs->size, kbm_ref_t, num_cmpgt(a.off, b.off));
	// estimate binmap
	...
	if(aux->refs->size){
		mr = aux->par->min_mat / (aux->par->ksize + aux->par->psize);
		if(mr < 512) mr = 512; //minimal alignment: 2*256
		aux->bmlen = tot / mr;
		if(aux->bmlen == 0) aux->bmlen = 1;
		aux->bmcnt = (aux->kbm->bins->size + aux->bmlen - 1) / aux->bmlen;
		if(aux->bmcnt < aux->qnbin * 50){
	...
	for(i=0;i<aux->refs->size;i++){
		...
		while(ref->boff < ref->bend){
			...
			hidx = ref->bidx / aux->bmcnt; //(first valid bid) / (match length)
			if(hidx - aux->bmoff < aux->nheap){ //hidx - (first match bin)
				#ifdef LOADDEBUG
				fprintf(KBM_LOGF, "hidx: %lu, bmoff: %lu, i: %lu\n", hidx, aux->bmoff, i);
				#endif
				push_u4v(aux->heaps[hidx - aux->bmoff], i);
			}
			break;
...
static inline int _backtrace_map_kbm(KBMAux *aux, int dir, kbm_path_t *p){
	KBMDP *dp;
	kbm_map_t *hit; //get the final hit
	kbm_cmer_t *c;
	u8i cgoff, sidx;
			...
			for(i=0;i<heap->size;i++){
				...
				while(1){
					...
					if(hidx > aux->hptr){
						if(hidx - aux->bmoff < aux->nheap){
							#ifdef LOADDEBUG
							fprintf(KBM_LOGF, "%s: hidx %lu, aux->bmoff %lu, idx %lu in thread %lu\n", __FUNCTION__, hidx, aux->bmoff, idx, pthread_self());
							#endif
